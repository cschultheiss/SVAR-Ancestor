...
rand_simulation <- function(nsim = 1000, n.vec = 10^(2:6), n.init = 10^4, p = 6, 
                            mc.cores = 16, h = 0, lingBoot = FALSE){
  ...
  # mc.cores (integer): number of cores for parallelization
  # h (integer): number of covariates to be hidden
  # linBoot (logical): whether to run bootstrapping LiNGAM (slow)
  ...
  # save which covariates are hidden from estimators
  if(h != 0){
  ...
  for (n in n.vec) {
    ...
    res<-foreach(gu = 1:nsim, .combine = rbind,
                 .packages = c("tsutils"), .options.snow = opts) %dorng%{
                   ...
                   # if p != 6 sample distributions randomly
                   ...
                   Lin <- lingam2.anc.ts(x[, -hidden[gu, ]], degree = nlag) # apply SVAR-LiNGAM (instant)
                   
                   # if linBoot = TRUE
                   # estimate the significance of SVAR-LiNGAM using bootstrap  
                   if(lingBoot){
                   ...
                   if(h == 0){
                     ...
                     b.val <- Lin
                     b2.val <- Lin3
                   }else{#collect results and add non-detections for the hidden covariates
